...
// --- مكون شاشة التحميل الافتتاحية (Initial Pre-loader) ---
const InitialLoader = ({ lang }) => {
  return (
    <motion.div 
      initial={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    ...
    >
      <div className="relative mb-8">
        {/* أيقونة الخلية العصبية مع نبض إشعاعي */}
        <motion.div 
      ...
      <div className="w-64">
        <div className="text-center text-[10px] font-bold text-teal-500 uppercase tracking-[0.3em] mb-3">
          {lang === 'ar' ? 'جاري تحميل النظام العصبي' : 'Initializing Neural System'}
        </div>
          ...
          return (
            <div key={index} className="relative">
              <div className="flex justify-between text-[10px] mb-1.5 px-1 font-medium items-center">
                ...
                <div className={`flex items-center gap-1 font-mono ${val > 0 ? 'text-rose-400' : 'text-emerald-400'}`} dir="ltr">
                  <span>{val > 0 ? '▲ +' : '▼ -'}</span>
...
const contentEn = { brand: "NeuroScan AI", langBtn: "AR", heroTitle: "Breast Cancer Classification Engine", heroDesc: "Analyze clinical cell nuclei features in real-time Powered by Deep Learning and Explainable AI", inputsTitle: "Clinical Parameters", fields: [{ id: 'worst_radius', label: 'Worst Radius' }, { id: 'worst_texture', label: 'Worst Texture' }, { id: 'worst_concave_points', label: 'Worst Concave Pts' }, { id: 'worst_area', label: 'Worst Area' }, { id: 'worst_concavity', label: 'Worst Concavity' }], btnAnalyze: "Run Diagnostic Analysis", btnLoading: "AI is Thinking", output: "Diagnostic Output", malignant: "Malignant", benign: "Benign", confidence: "Prediction Confidence", disclaimer: "For research use only Not a medical substitute", aboutTitle: "How It Works", aboutDesc: "NeuroScan AI bridges the gap between deep learning and clinical trust We utilize a multi-layer Neural Network combined with SHAP to ensure every prediction is transparent and interpretable", contactTitle: "Developer Support", contactDesc: "Encountered a bug or have questions Feel free to reach out", emailLabel: "Email Support", githubLabel: "View Source Code", linkedinLabel: "Professional Network", footer: "2026 NeuroScan AI Developed by Hassan Ahmed" }
const contentAr = { brand: "NeuroScan AI", langBtn: "EN", heroTitle: "محرك تصنيف أورام الثدي", heroDesc: "تحليل الخصائص السريرية لأنوية الخلايا لحظيا مدعوم بالتعلم العميق والذكاء الاصطناعي التفسيري", inputsTitle: "المعلمات السريرية", fields: [{ id: 'worst_radius', label: 'أقصى نصف قطر' }, { id: 'worst_texture', label: 'أقصى تباين للنسيج' }, { id: 'worst_concave_points', label: 'أقصى نقاط مقعرة' }, { id: 'worst_area', label: 'أقصى مساحة' }, { id: 'worst_concavity', label: 'أقصى تقعر' }], btnAnalyze: "بدء التحليل التشخيصي", btnLoading: "الذكاء الاصطناعي يفكر", output: "نتيجة التشخيص", malignant: "خبيث", benign: "حميد", confidence: "ثقة التنبؤ", disclaimer: "لأغراض البحث فقط ليس بديلا عن الاستشارة الطبية", aboutTitle: "كيف تعمل المنصة", aboutDesc: "تسد منصة نيورو سكان الفجوة بين التعلم العميق والثقة السريرية نستخدم شبكة عصبية متعددة الطبقات مع دمج تقنية شاب لضمان شفافية التنبؤ", contactTitle: "الدعم الفني والتواصل", contactDesc: "هل واجهت مشكلة تقنية او لديك استفسار لا تتردد في التواصل", emailLabel: "المراسلة عبر البريد", githubLabel: "الشيفرة المصدرية", linkedinLabel: "الشبكة المهنية", footer: "2026 NeuroScan AI تم التطوير بواسطة حسن أحمد" }
...
function App() {
  ...
  const [appReady, setAppReady] = useState(false) // حالة جاهزية التطبيق

  useEffect(() => {
    // محاكاة تحميل التطبيق لمدة 3 ثوانٍ
  ...
  const analyzeData = async (e) => {
    ...
    try {
      const response = await fetch('https://hassan2007-tumor-diagnosis-backend.hf.space/predict', {
        ...
        body: JSON.stringify({ worst_radius: parseFloat(formData.worst_radius), worst_texture: parseFloat(formData.worst_texture), worst_concave_points: parseFloat(formData.worst_concave_points), worst_area: parseFloat(formData.worst_area), worst_concavity: parseFloat(formData.worst_concavity) }),
      })
      const data = await response.json()
      clearInterval(interval); setProgress(100)
      setTimeout(() => { setResult(data); setLoading(false) }, 500)
    } catch (err) { clearInterval(interval); setLoading(false); alert("Error connecting to AI Server") }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 selection:bg-teal-500/30 relative overflow-hidden">
      ...
      <AnimatePresence>
        {!appReady && <InitialLoader lang={lang} />}
      </AnimatePresence>
              ...
              <div className="grid md:grid-cols-2 gap-5">
                {content.fields.map(f => (
                  <div key={f.id} className="group">
                    ...
                    <input type="number" step="any" required dir="ltr" value={formData[f.id]} onChange={e => setFormData({...formData, [f.id]: e.target.value})} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition-all text-left rtl:text-right [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
                  </div>
                ))}
              </div>
            ...
            {!result && !loading ? (
              ...
              <motion.div key="loading" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="h-[400px] border border-slate-700 bg-slate-900/80 rounded-3xl flex flex-col items-center justify-center text-center p-10 backdrop-blur-xl relative overflow-hidden shadow-2xl">
                ...
                <div className="relative z-10 w-full max-w-xs flex flex-col items-center">
                  <div className="text-[10px] font-bold uppercase text-teal-400 mb-4 tracking-widest">{lang === 'en' ? 'Scanning Clinical Parameters' : 'جاري فحص المعلمات السريرية'}</div>
